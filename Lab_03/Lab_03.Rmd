...
Hi guys! Welcome to week three. This week, we're going to review some materials on homework 1 that were quite tricky. In particualr regarding number 3, how to derive z-stat/t-stat and how to interpret the interaction term in a regression result. 

We're going to quickly go ahead and do the follwing; 
 - loading packages, 
 - go over some group statistics,
 - running regressions (with a twist),
 - And doing some analysis/thinking on those things. 

## Basic Setup

First, we do some basic set up;
  - install the relevant packages (if you have not done so already), 
  - load the relevent package, 
  - and import .csv file on r setting. 
```{r}
...
my_path = "C:/Users/boyoon/Dropbox/EC421/ps1/PS1_Data.csv"
dataPS01 <- read_csv(my_path)
```

## Group Statistics

Second, we do calculation on group statistics. 
...
To be clear, our estimating equation is: $$callback_i = B0 + B1*black_i + e_i $$
...
```{r}
call_f_model = lm(i_callback~i_black,data=dataPS01)
...
callback_i = a0+a1*exp_i+a2*black_i+a3*exp_i*black_i+e_i

```{r}
# Note where the : shows up between female and black at the end
interaction_reg= lm(i_callback ~ n_expr+i_black+n_expr:i_black, data=dataPS01)
...
For the first group (white with no experience) the estimating equation becomes 
 - callback_i = a0+e_i
 - a0 will indicate the callback rate of our reference group, in this case white with no experience
For the second group (white with experience) the estimating equation becomes
 - callback_i = a0+a1*exp_i+e_i
